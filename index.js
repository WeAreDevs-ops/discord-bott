...
function isBotOwner(userId) {
    const BOT_OWNER_ID = process.env.ADMIN_ID; // Use environment variable
...
async function saveLockedChannels(guildId, lockedChannelIds) {
  try {
    await db.ref(`guilds/${guildId}/lockedChannels`).set(lockedChannelIds);
    console.log(`Saved locked channels for ${guildId}`);
  } catch (error) {
    console.error('Error saving locked channels:', error);
  }
}

async function getLockedChannels(guildId) {
  try {
    const snapshot = await db.ref(`guilds/${guildId}/lockedChannels`).once('value');
    return snapshot.val() || [];
  } catch (error) {
    console.error('Error getting locked channels:', error);
    return [];
  }
}
  ...
  try {
    ...
    for (const [guildId, guildData] of Object.entries(guildsData)) {
      ...
      if (guildData.lockedChannels) {
        lockedChannels.set(guildId, new Set(guildData.lockedChannels));
      }
    }
...
function startStatusMonitoring() {
  ...
  if (!statusChannelId) {
    console.log('STATUS_CHANNEL_ID not set, skipping status monitoring');
    return;
  }
...
client.on('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('lockchannel')
      .setDescription('Lock or unlock a channel to prevent/allow messages (Admin/Owner only)')
      .addChannelOption(option =>
        option.setName('channel')
      ...
      .addStringOption(option =>
        option.setName('action')
          ...
          .setRequired(true)
          .addChoices(
            { name: 'Lock - Prevent all messages', value: 'lock' },
            { name: 'Unlock - Allow messages', value: 'unlock' },
            { name: 'Status - Check current status', value: 'status' }
          )
      )
      .addStringOption(option =>
        option.setName('reason')
          .setDescription('Reason for locking/unlocking the channel')
          .setRequired(false)
      )
      .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    new SlashCommandBuilder()
...
client.on('messageCreate', async message => {
  ...
  if (message.channelId === process.env.AUTO_REPLY_CHANNEL_ID) {
  ...
  // Note: Locked channels are now handled via Discord permissions, no need for auto-deletion
...
// Locked channels (guildId -> Set of channelIds)
...
async function reloadGuildSettings(guildId) {
  try {
    ...
    const lockedChannelsData = await getLockedChannels(guildId);
    if (lockedChannelsData && Array.isArray(lockedChannelsData) && lockedChannelsData.length > 0) {
      lockedChannels.set(guildId, new Set(lockedChannelsData));
    }
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'lockchannel') {
    ...
    if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageChannels)) {
      return interaction.reply({
        content: '<:no:1393890945929318542> I don\'t have permission to manage channel permissions.',
        ephemeral: true
      });
    }

    // Get or create locked channels set for this guild
    let guildLockedChannels = lockedChannels.get(interaction.guild.id);
    if (!guildLockedChannels) {
      guildLockedChannels = new Set();
      lockedChannels.set(interaction.guild.id, guildLockedChannels);
    }

    const isCurrentlyLocked = guildLockedChannels.has(channel.id);
    ...
    switch (action) {
      case 'lock':
        if (isCurrentlyLocked) {
          return interaction.reply({
            content: `<:no:1393890945929318542> ${channel} is already locked.`,
            ephemeral: true
          });
        }
        ...
        try {
          ...
          for (const [roleId, role] of roles) {
            // Skip @everyone and roles that are higher than or equal to bot's highest role
            if (role.name === '@everyone') continue;
            if (role.position >= interaction.guild.members.me.roles.highest.position) continue;
            
            // Skip administrator roles (they should be able to bypass)
            if (role.permissions.has(PermissionFlagsBits.Administrator)) continue;

            try {
              // Remove SEND_MESSAGES permission for this role in this channel
          ...
          // Also handle @everyone role separately
          try {
            ...
            console.error('Failed to update permissions for @everyone:', error);
            errorCount++;
          }

          guildLockedChannels.add(channel.id);
          await saveLockedChannels(interaction.guild.id, Array.from(guildLockedChannels));

          // Send embed message to the locked channel
          try {
            const lockEmbed = new EmbedBuilder()
              .setColor(0xff4757)
              .setTitle('🔒 Channel Locked')
              .setDescription(reason !== 'No reason provided' ? reason : '')
              .setTimestamp();

            await channel.send({ embeds: [lockEmbed] });
          } catch (error) {
            console.error('Error sending lock message to channel:', error);
          }

          embed = new EmbedBuilder()
            .setColor(0xff4757)
            .setTitle('🔒 Channel Locked')
            .setDescription(`${channel} has been locked by removing send message permissions.`)
            .addFields(
              { name: 'Channel', value: `${channel}`, inline: true },
              { name: 'Status', value: '🔒 Locked', inline: true },
              { name: 'Reason', value: reason, inline: false },
              { name: 'Permissions Updated', value: `${successCount} roles affected`, inline: true },
              { name: 'Bypassed by', value: 'Server Owner and Administrator roles', inline: false }
            )
            .setTimestamp()
            .setFooter({
          ...
          if (errorCount > 0) {
            embed.addFields({
              name: 'Warning', 
              value: `${errorCount} role permissions couldn't be updated (higher hierarchy)`, 
              inline: false
            });
          }
          ...
          await interaction.reply({
            content: '<:no:1393890945929318542> Failed to lock channel. I may lack permissions to manage channel permissions.',
            ephemeral: true
          });
        }
        break;
      ...
      case 'unlock':
        if (!isCurrentlyLocked) {
          return interaction.reply({
            content: `<:no:1393890945929318542> ${channel} is not currently locked.`,
            ephemeral: true
          });
        }

        try {
          // Get existing permission overwrites for this channel
          const existingOverwrites = channel.permissionOverwrites.cache;
          let successCount = 0;
          let errorCount = 0;

          // Remove permission overwrites that were created during lock
          for (const [targetId, overwrite] of existingOverwrites) {
            try {
              // Check if this overwrite denies SendMessages
              if (overwrite.deny.has(PermissionFlagsBits.SendMessages)) {
                // Delete the entire permission overwrite to restore defaults
                await overwrite.delete();
                successCount++;
              }
            } catch (error) {
              console.error(`Failed to remove permission overwrite for ${targetId}:`, error);
              errorCount++;
            }
          }

          // If no overwrites were found/removed, try the fallback method
          if (successCount === 0) {
            // Fallback: explicitly reset @everyone permissions
            try {
              ...
              console.error('Failed to remove @everyone overwrite:', error);
              errorCount++;
            }
          }

          guildLockedChannels.delete(channel.id);
          await saveLockedChannels(interaction.guild.id, Array.from(guildLockedChannels));

          embed = new EmbedBuilder()
            .setColor(0x00ff88)
            .setTitle('🔓 Channel Unlocked')
            .setDescription(`${channel} has been unlocked by removing permission restrictions.`)
            .addFields(
              { name: 'Channel', value: `${channel}`, inline: true },
              { name: 'Status', value: '🔓 Unlocked', inline: true },
              { name: 'Reason', value: reason, inline: false },
              { name: 'Permissions Restored', value: `${successCount} overwrite${successCount !== 1 ? 's' : ''} removed`, inline: true },
              { name: 'Messages', value: 'All message types are now allowed', inline: false }
            )
            .setTimestamp()
            .setFooter({
          ...
          if (errorCount > 0) {
            embed.addFields({
              name: 'Warning', 
              value: `${errorCount} permission${errorCount !== 1 ? 's' : ''} couldn't be removed`, 
              inline: false
            });
          }
          ...
          await interaction.reply({
            content: '<:no:1393890945929318542> Failed to unlock channel. I may lack permissions to manage channel permissions.',
            ephemeral: true
          });
        }
        break;
      ...
      case 'status':
        const totalLocked = guildLockedChannels.size;
        const lockedChannelsList = totalLocked > 0 ? 
          Array.from(guildLockedChannels).slice(0, 10).map(id => `<#${id}>`).join('\n') + 
          (totalLocked > 10 ? `\n+${totalLocked - 10} more` : '') :
          'No channels are currently locked';

        embed = new EmbedBuilder()
          .setColor(isCurrentlyLocked ? 0xff4757 : 0x0ea5e9)
          .setTitle('🔒 Channel Lock Status')
          .setDescription(`Status for ${channel}`)
          .addFields(
            { name: 'Current Status', value: isCurrentlyLocked ? '🔒 Locked (Permissions Denied)' : '🔓 Unlocked (Messages Allowed)', inline: true },
            { name: 'Total Locked Channels', value: `${totalLocked}`, inline: true },
            { name: 'Locked Channels', value: lockedChannelsList, inline: false }
          )
          .setTimestamp()
          .setFooter({
            text: `Checked by ${interaction.user.tag}`,
