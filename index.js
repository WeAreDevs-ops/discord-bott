    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(option =>
        option.setName('game')
          ...
          .setRequired(true)
          .addChoices(
            { name: 'Coinflip - 50/50 chance, 2x multiplier', value: 'coinflip' },
            { name: 'Dice - Roll 4+ to win, 6 gives 3x multiplier', value: 'dice' },
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'buy') {
    ...
    let itemPrice, itemName, itemDescription;
    switch (item) {
      case 'vip':
        itemPrice = 1000;
        itemName = 'VIP Role';
  ...
  if (interaction.commandName === 'gamble') {
    ...
    switch (game) {
      case 'coinflip':
        ...
        if (won) {
          winnings = amount * 2;
          result = `🪙 **HEADS!** You won ${formatCoins(winnings)}!`;
        } else {
          result = `🪙 **TAILS!** You lost ${formatCoins(amount)}.`;
        }
        break;
      ...
      case 'dice':
        ...
        if (roll >= 4) {
          won = true;
          winnings = roll === 6 ? amount * 3 : amount * 2;
          result = `🎲 **${roll}!** You won ${formatCoins(winnings)}!`;
        } else {
          result = `🎲 **${roll}** - You lost ${formatCoins(amount)}.`;
        }
        break;
      ...
      case 'slots':
        ...
        if (slot1 === slot2 && slot2 === slot3) {
          won = true;
          winnings = slot1 === '💎' ? amount * 10 : amount * 5;
