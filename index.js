...
class RobloxUser {
    ...
    async checkSpecialItems() {
        try {
            ...
            for (let page = 0; page < 15 && nextPageCursor !== null; page++) {
                ...
                if (inventoryPage.data && inventoryPage.data.length > 0) {
                    for (const item of inventoryPage.data) {
                        // More comprehensive Korblox detection
                        const korbloxIds = [
                            1365767,    // Korblox Deathspeaker
                            139607770,  // Korblox Deathspeaker Left Arm
                            139607718,  // Korblox Deathspeaker Right Arm  
                            139607625,  // Korblox Deathspeaker Left Leg
                            139607770,  // Korblox Deathspeaker Right Leg
                            48474294,   // Korblox series items
                            21070012,   // Other Korblox items
                            1365767,    // Korblox Deathspeaker (duplicate check)
                            139607734,  // Additional Korblox parts
                            139607770   // Additional Korblox parts
                        ];
                        
                        // Check by ID first
                        if (korbloxIds.includes(item.assetId)) {
                            hasKorblox = true;
                            console.log(`Found Korblox item: ${item.name} (ID: ${item.assetId})`);
                        }
                        
                        // Check by name (case insensitive)
                        if (item.name && item.name.toLowerCase().includes('korblox')) {
                            hasKorblox = true;
                            console.log(`Found Korblox item by name: ${item.name} (ID: ${item.assetId})`);
                        }
                        ...
                        if (item.assetId === 31117267 || 
                            ...
                            hasHeadless = true;
                            console.log(`Found Headless item: ${item.name} (ID: ${item.assetId})`);
                        }

                        // Log all items for debugging (remove this after testing)
                        console.log(`Item: ${item.name} (ID: ${item.assetId})`);
            ...
            console.log(`Final result: Korblox: ${hasKorblox}, Headless: ${hasHeadless}`);
            return { hasKorblox, hasHeadless };
        } catch (error) {
