        ...
        try {
            if (response.body && typeof response.body === 'string' && response.body.includes('�')) {
                ...
                try {
                    ...
                    console.log("✅ Challenge response decompressed successfully");
                } catch (decompError) {
                    console.log("❌ Failed to decompress challenge response");
                    challengeResult = { error: "Failed to decompress response" };
                }
            } else {
        ...
        try {
            ...
            if (response.body && typeof response.body === 'string' && response.body.includes('�')) {
                ...
                try {
                    ...
                    // Try gzip first
                    try {
                        decompressed = zlib.gunzipSync(Buffer.from(response.body, 'binary'));
                    } catch (gzipError) {
                        // Try deflate if gzip fails
                        try {
                            decompressed = zlib.inflateSync(Buffer.from(response.body, 'binary'));
                        } catch (deflateError) {
                            throw new Error("Failed to decompress with both gzip and deflate");
                        }
                    }
                    
                    const decompressedText = decompressed.toString();
                    console.log("✅ Successfully decompressed response:", decompressedText);
                    
                    responseData = decompressedText ? JSON.parse(decompressedText) : {};
                } catch (decompError) {
                    console.log("❌ Failed to decompress response:", decompError.message);
                    responseData = { 
                        error: "Response appears to be compressed but failed to decompress",
