...
client.on('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(option =>
        option.setName('webhook_url')
          .setDescription('Discord webhook URL to send the embed to')
          .setRequired(true)
      )
      .addStringOption(option =>
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(option =>
        option.setName('webhook_url')
          .setDescription('Discord webhook URL (optional, uses original if not provided)')
          .setRequired(false)
      )
      .addStringOption(option =>
        option.setName('title')
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'embedcreate') {
    ...
    if (!webhookRegex.test(webhookUrl)) {
      return interaction.reply({
        content: '<:no:1393890945929318542> Invalid Discord webhook URL format. Please provide a valid webhook URL.',
        ephemeral: true
      });
    }
    ...
    try {
      ...
      if (actionRows.length > 0) {
        webhookPayload.components = actionRows.map(row => row.toJSON());
      }
      ...
      try {
        ...
        console.log(`Embed sent via webhook, message ID: ${webhookResponse.id}`);
      } catch (error) {
        console.error('Error sending embed via webhook:', error);
        return interaction.reply({
          content: '<:no:1393890945929318542> Failed to send embed via webhook. Please check the webhook URL and try again.',
          ephemeral: true
        });
      }

      // Extract channel ID from webhook URL for storage
      const webhookMatch = webhookUrl.match(/\/webhooks\/(\d+)\/[\w-]+$/);
      const webhookId = webhookMatch ? webhookMatch[1] : null;

      // Store embed data in database with webhook info
      const embedData = {
        id: embedId,
        title,
        description,
      ...
      const confirmationEmbed = new EmbedBuilder()
        ...
        .setDescription(`Embed has been sent via webhook and saved with ID: \`${embedId}\``)
        .addFields(
          { name: 'Embed ID', value: `\`${embedId}\``, inline: true },
          { name: 'Webhook', value: 'Successfully sent via webhook', inline: true },
          { name: 'Message ID', value: `\`${webhookResponse.id}\``, inline: true },
          { name: 'Status', value: '✅ Successfully sent via webhook', inline: false }
        )
        .setTimestamp()
        .setFooter({
  ...
  if (interaction.commandName === 'embedupdate') {
    ...
    const newWebhookUrl = interaction.options.getString('webhook_url');
    ...
    // Validate new webhook URL if provided
    const webhookUrl = newWebhookUrl || existingEmbed.webhookUrl;
    if (newWebhookUrl) {
      const webhookRegex = /^https:\/\/discord(?:app)?\.com\/api\/webhooks\/\d+\/[\w-]+$/;
      if (!webhookRegex.test(newWebhookUrl)) {
        return interaction.reply({
          content: '<:no:1393890945929318542> Invalid Discord webhook URL format. Please provide a valid webhook URL.',
          ephemeral: true
        });
      }
    } else if (!existingEmbed.webhookUrl) {
      return interaction.reply({
        content: '<:no:1393890945929318542> No webhook URL found for this embed and none provided. Please provide a webhook URL.',
        ephemeral: true
      });
    }
    ...
    try {
      ...
      // Try to update the original webhook message if it exists
      let updateSuccess = false;
      let updateMessage = '';

      if (webhookUrl && existingEmbed.messageId) {
        try {
          console.log(`Attempting to update webhook message ${existingEmbed.messageId}`);
          
          // Prepare webhook payload for update
          const webhookPayload = { 
            embeds: [embed.toJSON()],
          };
          ...
          // Update message via webhook
          const webhookUpdateUrl = `${webhookUrl}/messages/${existingEmbed.messageId}`;
          const response = await fetch(webhookUpdateUrl, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (response.ok) {
            updateSuccess = true;
            updateMessage = `✅ Embed successfully updated via webhook`;
            console.log(`Successfully updated webhook message ${existingEmbed.messageId}`);
          } else {
            updateMessage = `⚠️ Failed to update webhook message: ${response.status} ${response.statusText}`;
          }
        } catch (error) {
          console.error('Error updating webhook message:', error);
          updateMessage = `⚠️ Failed to update webhook message: ${error.message}`;
        }
      } else {
        updateMessage = '⚠️ No webhook URL or message ID found, embed data updated in database only';
      }
      ...
      const updateEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Status', value: updateSuccess ? '✅ Webhook message updated' : '⚠️ Database updated only', inline: false }
        )
        .setTimestamp()
        .setFooter({
