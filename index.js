...
client.on('ready', async () => {
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('embedcreate')
      .setDescription('Create a custom embed message with clickable buttons')
      .addStringOption(option =>
      ...
      .addStringOption(option =>
        option.setName('buttons')
          .setDescription('Buttons format: "Label1,URL1|Label2,URL2|Label3,URL3" (max 5 buttons)')
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName('stats')
    ...
    new SlashCommandBuilder()
      .setName('autorole')
      .setDescription('Configure automatic role assignment when users join (Requires Administrator)')
      .addStringOption(option =>
        option.setName('mode')
      ...
      if (buttonsInput) {
        try {
          ...
          for (const pair of buttonPairs) {
            ...
            if (!label || !url) {
              return interaction.reply({
                content: '<:no:1393890945929318542> Invalid button format. Use: "Label1,URL1|Label2,URL2"',
                ephemeral: true
              });
            }
            ...
            try {
              ...
              return interaction.reply({
                content: `<:no:1393890945929318542> Invalid URL for button "${label}": ${url}`,
                ephemeral: true
              });
            }
            ...
            if (label.length > 80) {
              return interaction.reply({
                content: `<:no:1393890945929318542> Button label "${label}" is too long (max 80 characters).`,
                ephemeral: true
              });
            }
