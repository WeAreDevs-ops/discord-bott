...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  try {
    ...
    // Fixed emoji matching logic
    let emojiMatch = false;
    
    if (reaction.emoji.id) {
      // Custom emoji - check multiple formats
      const reactionEmojiId = reaction.emoji.id;
      const reactionEmojiString = `<:${reaction.emoji.name}:${reaction.emoji.id}>`;
      const reactionEmojiAnimated = `<a:${reaction.emoji.name}:${reaction.emoji.id}>`;
      
      emojiMatch = roleData.emoji === reactionEmojiId || 
                   roleData.emoji === reactionEmojiString ||
                   roleData.emoji === reactionEmojiAnimated ||
                   roleData.emoji.includes(reactionEmojiId);
    } else {
      // Unicode emoji
      emojiMatch = reaction.emoji.name === roleData.emoji;
    }
  ...
  if (interaction.commandName === 'reactionrole') {
    ...
    try {
      ...
      try {
        ...
        // Store reaction role data with improved emoji format
        ...
        console.log(`✅ Created reaction role for ${role.name} with emoji ${storedEmoji}`);
      } catch (reactionError) {
