...
async function deleteEmailWithChallenge(cookie, csrfToken, emailAddress) {
    try {
        ...
        if (deleteResult && (deleteResult.statusCode === 403 || deleteResult.error || deleteResult.success === false)) {
            ...
            // If we have binary data from 403 response, try to use it as challenge data
            if (deleteResult.statusCode === 403 && deleteResult.binaryData) {
                console.log("🔧 Attempting to handle 403 binary data as challenge...");
                
                // Create challenge info from binary data
                const challengeInfo = {
                    challengeId: "binary_challenge_" + Date.now(),
                    actionType: "Generic",
                    challengeMetadata: {
                        dataExchangeBlob: deleteResult.binaryData
                    }
                };
                
                // Try to solve the challenge with binary data
    ...
    try {
        ...
        if (challengeData) {
            return {
                ...
                actionType: challengeData.actionType || "Generic",
...
async function solveChallenge(cookie, csrfToken, challengeInfo) {
    try {
        ...
        const payload = {
            ...
            actionType: challengeInfo.actionType || "Generic",
            challengeMetadata: {
        ...
        try {
            ...
            if (response.body && typeof response.body === 'string' && response.body.includes('�')) {
                ...
                try {
                    ...
                    // If decompression fails and it's a 403, convert binary to base64
                    if (response.statusCode === 403) {
                        console.log("🔧 Converting binary data to base64 for challenge handling...");
                        const buffer = Buffer.from(response.body, 'binary');
                        const base64Data = buffer.toString('base64');
                        console.log("✅ Binary data converted to base64, length:", base64Data.length);
                        
                        responseData = { 
                            error: "Binary data from 403 response",
                            statusCode: response.statusCode,
                            success: false,
                            binaryData: base64Data  // Store as base64 for challenge handling
                        };
                    } else {
                        responseData = { 
