...
client.once('ready', async () => {
  ...
  // Sync current guilds to Firebase
  try {
    console.log('Syncing current guild data to Firebase...');
    ...
    console.log(`Synced ${syncedCount} guilds to Firebase on startup`);
  } catch (error) {
    console.error('Error syncing guilds to Firebase:', error);
  }
...
// Real-time Firebase listeners for live updates from dashboard
function setupRealtimeListeners() {
  console.log('Setting up real-time Firebase listeners...');
  
  // Listen for changes to all guild settings
  db.ref('guilds').on('child_changed', (snapshot) => {
    const guildId = snapshot.key;
    const guildData = snapshot.val();
    
    console.log(`🔄 Real-time update for guild ${guildId}`);
    
    // Update guild settings (welcome/leave)
    if (guildData.settings) {
      guildSettings.set(guildId, guildData.settings);
    ...
    if (guildData.automod) {
      autoModSettings.set(guildId, guildData.automod);
      console.log(`Updated automod settings for ${guildId}`);
    } else {
    ...
    if (guildData.commandAssignments) {
      commandChannelAssignments.set(guildId, guildData.commandAssignments);
      console.log(`Updated command assignments for ${guildId}`);
    }
    
    // Update auto role settings
    if (guildData.autorole) {
      autoRoleSettings.set(guildId, guildData.autorole);
      console.log(`Updated auto role for ${guildId}`);
    } else {
      autoRoleSettings.delete(guildId);
    }
    
    // Update restricted channels
    if (guildData.restrictedChannels) {
      restrictedChannels.set(guildId, new Set(guildData.restrictedChannels));
      console.log(`Updated restricted channels for ${guildId}`);
    } else {
      restrictedChannels.delete(guildId);
    }
  });
  
  // Listen for new guilds being added
  db.ref('guilds').on('child_added', (snapshot) => {
    const guildId = snapshot.key;
    const guildData = snapshot.val();
    
    // Only process if this is a new guild we haven't seen
    if (!guildSettings.has(guildId) && guildData.settings) {
      console.log(`📥 New guild settings loaded: ${guildId}`);
      guildSettings.set(guildId, guildData.settings);
    }
  });
  
  console.log('✅ Real-time Firebase listeners are active');
}
...
// Guild join event - when bot is added to a new server
client.on('guildCreate', async guild => {
  try {
    console.log(`Bot joined new guild: ${guild.name} (${guild.id})`);
    
    // Sync new guild to Firebase
    await db.ref(`guilds/${guild.id}`).update({
      name: guild.name,
      memberCount: guild.memberCount,
      ownerId: guild.ownerId,
    ...
    console.log(`Synced new guild ${guild.name} to Firebase`);
  } catch (error) {
    console.error(`Error syncing new guild ${guild.name} to Firebase:`, error);
  }
});

// Guild leave event - when bot is removed from a server
client.on('guildDelete', async guild => {
  try {
    console.log(`Bot left guild: ${guild.name} (${guild.id})`);
    
    // Update guild status in Firebase (mark as bot not present)
    await db.ref(`guilds/${guild.id}`).update({
      botPresent: false,
      leftAt: Date.now(),
      lastUpdated: Date.now()
    ...
    console.log(`Updated guild ${guild.name} status in Firebase (bot left)`);
  } catch (error) {
    console.error(`Error updating guild ${guild.name} status in Firebase:`, error);
  }
});
...
app.get('/api/server/:serverId/discord-data', async (req, res) => {
  ...
  try {
    ...
    try {
      // Fetch all text channels (type 0)
      ...
      console.log(`✅ Fetched ${channels.length} channels and ${roles.length} roles for ${guild.name}`);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { client, commandStats };
}

// Export bot data to server for admin dashboard
client.once('ready', () => {
  ...
  client.guilds.cache.forEach(guild => {
    console.log(`✅ Bot present in: ${guild.name} (${guild.id}) - ${guild.memberCount} members`);
  });
  
  // Set bot data after a delay to ensure server is loaded
  setTimeout(() => {
    try {
      const serverModule = require('./server.js');
      if (serverModule && serverModule.setBotData) {
        serverModule.setBotData(client, commandStats);
        console.log('✅ Bot data connected to server dashboard');
      }
    } catch (error) {
      console.log('Server module not loaded yet, bot data will be available after server starts');
    }
  }, 3000);
});