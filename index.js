...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  try {
    ...
    // Improved emoji matching
    let emojiMatch = false;
    
    if (reaction.emoji.id) {
      // Custom emoji
      emojiMatch = reaction.emoji.id === roleData.emoji || 
                   `<:${reaction.emoji.name}:${reaction.emoji.id}>` === roleData.emoji ||
                   `<a:${reaction.emoji.name}:${reaction.emoji.id}>` === roleData.emoji;
    } else {
      // Unicode emoji
