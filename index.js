  ...
  if (interaction.commandName === 'embedupdate') {
    ...
    try {
      ...
      if (existingEmbed.channelId && existingEmbed.messageId) {
        try {
          const channel = interaction.guild.channels.cache.get(existingEmbed.channelId);
          if (channel) {
            const message = await channel.messages.fetch(existingEmbed.messageId);
            if (message) {
              await message.edit(messageOptions);
              updateSuccess = true;
              updateMessage = `Embed updated in ${channel}`;
            } else {
              updateMessage = 'Original message not found, embed data updated in database only';
            }
          } else {
            updateMessage = 'Original channel not found, embed data updated in database only';
          }
        } catch (error) {
          console.error('Error updating Discord message:', error);
          updateMessage = 'Failed to update Discord message (permissions or message deleted), but embed data was saved';
        }
      } else {
        updateMessage = 'No original message info found, embed data updated in database only';
      }
