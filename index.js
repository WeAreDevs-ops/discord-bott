...
client.once('ready', async () => {
  ...
  try {
    ...
    // Initialize automod settings for guilds that don't have them
    for (const guild of client.guilds.cache.values()) {
      if (!autoModSettings.has(guild.id)) {
        const defaultAutoMod = {
          linkFilter: false,
          badWordFilter: false,
          badWords: [...defaultBadWords]
        };
        autoModSettings.set(guild.id, defaultAutoMod);
        await saveAutoModSettings(guild.id, defaultAutoMod);
        console.log(`Initialized default automod settings for guild: ${guild.name} (${guild.id})`);
      }
    }
...
client.on('guildCreate', async guild => {
  ...
  try {
    // Initialize default automod settings for new guild
    const defaultAutoMod = {
      linkFilter: false,
      badWordFilter: false,
      badWords: [...defaultBadWords]
    ...
    console.log(`Initialized automod settings for new guild: ${guild.name} (${guild.id})`);
    
    // Initialize empty guild settings
    const defaultGuildSettings = {
      welcomeChannel: null,
      leaveChannel: null,
      welcomeMessages: ['Welcome {user} to {server}! You are the {membercount} member!'],
      leaveMessages: ['{username} has left {server}. We\'ll miss you! 👋']
    };
    
    guildSettings.set(guild.id, defaultGuildSettings);
    await saveGuildSettings(guild.id, defaultGuildSettings);
    
    console.log(`Initialized guild settings for new guild: ${guild.name} (${guild.id})`);
    
  } catch (error) {
    console.error(`Error initializing settings for new guild ${guild.name}:`, error);
  }
});
