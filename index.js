...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'embedcreate') {
    ...
    // Check if bot has permission to send messages and embed links in the target channel
    ...
    if (!channelPermissions.has(PermissionFlagsBits.SendMessages)) {
      return interaction.reply({
        content: `<:no:1393890945929318542> I don't have permission to send messages in ${targetChannel}.`,
        ephemeral: true
      });
    }
    ...
    if (!channelPermissions.has(PermissionFlagsBits.EmbedLinks)) {
      return interaction.reply({
        content: `<:no:1393890945929318542> I don't have permission to embed links in ${targetChannel}.`,
        ephemeral: true
      });
    }
    ...
    try {
      ...
      try {
        ...
        console.log(`Embed sent to channel ${targetChannel.id}, message ID: ${sentMessage.id}`);
      } catch (error) {
        console.error('Error sending embed to Discord:', error);
        return interaction.reply({
          content: '<:no:1393890945929318542> Failed to send embed to the specified channel. Check bot permissions.',
          ephemeral: true
        });
      }

      // Store embed data in database with message info
      const embedData = {
        id: embedId,
        title,
        description,
      ...
      const confirmationEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Status', value: '✅ Successfully sent to Discord', inline: false }
        )
        .setTimestamp()
        .setFooter({
  ...
  if (interaction.commandName === 'embedupdate') {
    ...
    try {
      ...
      // Try to update the original Discord message if it exists
      let updateSuccess = false;
      let updateMessage = '';

      if (existingEmbed.channelId && existingEmbed.messageId) {
        try {
          console.log(`Attempting to update message ${existingEmbed.messageId} in channel ${existingEmbed.channelId}`);
          
          const channel = await interaction.guild.channels.fetch(existingEmbed.channelId);
          if (channel) {
            const message = await channel.messages.fetch(existingEmbed.messageId);
            if (message) {
              await message.edit(messageOptions);
              updateSuccess = true;
              updateMessage = `✅ Embed successfully updated in ${channel}`;
              console.log(`Successfully updated Discord message ${existingEmbed.messageId}`);
            } else {
              updateMessage = '⚠️ Original message not found, embed data updated in database only';
            }
          } else {
            updateMessage = '⚠️ Original channel not found, embed data updated in database only';
          }
        } catch (error) {
          console.error('Error updating Discord message:', error);
          if (error.code === 10008) {
            updateMessage = '⚠️ Original message was deleted, embed data updated in database only';
          } else if (error.code === 50013) {
            updateMessage = '⚠️ Missing permissions to edit message, embed data updated in database only';
          } else {
            updateMessage = `⚠️ Failed to update Discord message: ${error.message}`;
          }
        }
      } else {
        updateMessage = '⚠️ No original message info found, embed data updated in database only';
      }
      ...
      const updateEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Status', value: updateSuccess ? '✅ Discord message updated' : '⚠️ Database updated only', inline: false }
        )
        .setTimestamp()
        .setFooter({
