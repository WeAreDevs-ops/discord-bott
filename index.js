...
function canUseCommandInChannel(guildId, channelId, commandName) {
    ...
    const assignedChannel = guildAssignments[commandName];
    if (!assignedChannel) return true; // Command not assigned = allow everywhere
    
    return assignedChannel === channelId;
}
...
client.on('interactionCreate', async interaction => {
  ...
  // Allow commandassign to be used anywhere by bot/server owners
  if (commandName === 'commandassign') {
    ...
    if (!canUseHere) {
      ...
      const assignedChannelId = guildAssignments?.[commandName];
      const channelMention = assignedChannelId ? `<#${assignedChannelId}>` : 'the designated channel';
  ...
  if (interaction.commandName === 'commandassign') {
    ...
    if (mode === 'auto') {
      ...
      const guildAssignments = commandChannelAssignments.get(interaction.guild.id) || {};
      const currentAssignment = guildAssignments[commandName];

      const embed = new EmbedBuilder()
        .setColor(0x00d4ff)
        .setTitle(`📋 Available Channels for /${commandName}`)
        .setDescription(`Select any channel below to assign **/${commandName}** to it.\nCurrently assigned to: ${currentAssignment ? `<#${currentAssignment}>` : 'None (usable anywhere)'}`)
        .setTimestamp()
        .setFooter({
      ...
      channelChunks.forEach((chunk, index) => {
        const channelList = chunk.map(channel => {
          const isAssigned = currentAssignment === channel.id;
      ...
      // Assign command to channel
      guildAssignments[commandName] = targetChannel.id;
