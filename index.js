...
async function getRoastReply(userMessage, username) {
    try {
        ...
        // Create a roast-style prompt for the model
        const roastPrompt = `User ${username} said: "${userMessage}". Give a witty, sarcastic comeback:`;
        ...
        let reply = data?.generated_text || data?.[0]?.generated_text || data?.[0]?.translation_text;
        
        // Clean up the reply by removing the original prompt if it's included
        if (reply && reply.includes(roastPrompt)) {
            reply = reply.replace(roastPrompt, '').trim();
        }
