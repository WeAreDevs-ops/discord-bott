...
async function getOpenAIResponse(userMessage, username) {
    try {
        // Check if API key exists
        if (!process.env.OPENAI_API_KEY) {
            console.error('❌ OPENAI_API_KEY not found in environment variables');
            return "My brain is missing! Someone forgot to give me an API key! 🧠";
        }
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a sarcastic, toxic, funny Discord bot. Your job is to roast users, argue back, or make smart, funny replies. Never be helpful. Be unpredictable and witty. Keep responses under 200 characters. Be creative with your insults and comebacks.'
                    },
                    {
                        role: 'user',
                        content: `A user named ${username} just said: "${userMessage}". Give them a clever, funny roast or sarcastic comeback.`
                    }
                ],
                max_tokens: 100,
                temperature: 0.9,
        ...
        if (!response.ok) {
            if (response.status === 401) {
                ...
                console.error('❌ OpenAI API Error: Insufficient credits');
                return "I'm broke! Someone needs to pay my bills! 💸";
            } else if (response.status === 404) {
                console.error('❌ OpenAI API Error: Model not found');
                return "My brain model got lost in space! 🚀";
            } else {
        ...
        if (!reply) {
            console.error('❌ No reply from Hugging Face API');
            return "I'm too busy roasting other people right now. Try again later! 🔥";
        }
...
client.on('messageCreate', async message => {
  ...
  if (message.channelId === '1393907304327413851') {
    ...
    try {
      ...
      // Get OpenAI response
      const roastMessage = await getOpenAIResponse(message.content, message.author.username);
