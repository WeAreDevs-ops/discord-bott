...
async function getOpenAIResponse(userMessage, username) {
    try {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a witty, sarcastic Discord bot that gives smart roasts, funny arguments, or sarcastic replies. Keep responses under 200 characters and make them entertaining but not too harsh.'
                    },
                    {
                        role: 'user',
        ...
        if (!response.ok) {
            ...
            return "My roast generator is having a moment. Maybe that's for the best! 😅";
        }
        ...
        if (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
            ...
            return "I'm speechless... and that never happens! 🤐";
        }
    } catch (error) {
        console.error('OpenAI API error:', error);
        return "My wit.exe has stopped working. Please try again! 🤖";
    }
}
