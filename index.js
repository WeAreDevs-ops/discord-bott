...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'bypass2008') {
    ...
    // Get basic user info (username and avatar only) FIRST
    ...
    try {
      ...
      if (userRes.ok) {
        ...
        // Get avatar
        const avatarRes = await fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userData.id}&size=150x150&format=Png`);
        if (avatarRes.ok) {
    ...
    try {
      // Now get the bypass result with better error handling
      ...
      if (!contentType || !contentType.includes('application/json')) {
        throw new Error('API returned non-JSON response (possibly blocked or down)');
      }
      ...
      // Still show user info even if bypass fails
      const embed = new EmbedBuilder()
        .setColor(0xfacc15)
        .setTitle(`🎯 ${username}`)
        .setDescription("📧 **Email Bypass (2008 Method)**")
      ...
      embed.addFields(
        ...
        { name: "Suggestion", value: "The bypass API might be down or blocked. Try again later.", inline: false }
      );
