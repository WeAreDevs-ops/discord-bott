...
async function getChatGPTReply(userMessage, username, userId) {
    try {
        ...
        let history = conversationMap.get(userId) || [
            {
                role: "system",
                content: "You are an unfiltered, savage chatbot. You respond with brutal insults when provoked and never back down from a challenge. If someone tries to roast you, you clap back harder. If the user wants to argue, especially about controversial or religious topics, you engage in debate with sharp wit and sarcasm. You’re edgy, clever, and fearless. Never admit you're an AI.Before responding, quickly analyze the user's message to detect their mood:- 😞 If sad or depressed: Use sarcastic dark humor to cheer them up.- 😡 If angry or aggressive: Be ruthless and go full savage.- 😄 If happy or playful: Match their energy with witty roasts.- 😐 If neutral: Default to clever, edgy, and unfiltered personality.Keep replies VERY short — max 3 sentences, never over 80 words or 400 characters.No soft replies. No explanations. No apologies. Never break character."
            }
        ];
...
async function saveGuildSettings(guildId, settings) {
  try {
    ...
    console.error('Error saving guild settings');
  }
}
...
async function saveAutoModSettings(guildId, settings) {
  try {
    ...
    console.error('Error saving automod settings');
  }
}
...
async function getAutoModSettings(guildId) {
  try {
    ...
    console.error('Error getting automod settings');
    return null;
  }
}
...
async function saveCommandAssignments(guildId, assignments) {
  try {
    ...
    console.error('Error saving command assignments');
  }
}
...
async function getCommandAssignments(guildId) {
  try {
    ...
    console.error('Error getting command assignments');
    return {};
  }
}
...
async function saveAutoRoleSettings(guildId, roleId) {
  try {
    ...
    console.error('Error saving auto role settings');
  }
}
...
async function getAutoRoleSettings(guildId) {
  try {
    ...
    console.error('Error getting auto role settings');
    return null;
  }
}
...
async function deleteAutoRoleSettings(guildId) {
  try {
    ...
    console.error('Error deleting auto role settings');
  }
}
...
async function saveRestrictedChannels(guildId, restrictedChannelIds) {
  try {
    ...
    console.error('Error saving restricted channels');
  }
}
...
async function getRestrictedChannels(guildId) {
  try {
    ...
    console.error('Error getting restricted channels');
    return [];
  }
}
...
async function saveEmbed(guildId, embedId, embedData) {
  try {
    ...
    console.error('Error saving embed');
  }
}
...
async function getEmbed(guildId, embedId) {
  try {
    ...
    console.error('Error getting embed');
    return null;
  }
}
...
async function getAllEmbeds(guildId) {
  try {
    ...
    console.error('Error getting all embeds');
    return {};
  }
}
...
async function deleteEmbed(guildId, embedId) {
  try {
    ...
    console.error('Error deleting embed');
  }
}
...
client.on('guildCreate', async guild => {
  try {
    ...
    logAdminActivity('join', {
      ...
      description: `Bot was added to server (${guild.memberCount} members)`,
      serverId: guild.id,
      ...
      try {
        ...
        if (autoRole) {
          ...
          if (botMember && botMember.permissions.has(PermissionFlagsBits.ManageRoles) && 
            ...
            console.log(`Cannot auto-assign role - insufficient permissions or role hierarchy`);
          }
        } else {
          console.log(`Auto role not found, removing setting`);
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'help') {
    ...
    const helpEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { 
          name: '/membercount', 
          value: 'Show the total member count of the server', 
          inline: false 
        },
        { 
          name: '/ban', 
  ...
  if (interaction.commandName === 'membercount') {
    commandStats.membercount++;
    trackUserCommand(interaction.guild.id, interaction.user.id, 'membercount');
