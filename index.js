...
// Reaction roles (messageId -> { guildId: string, roleId: string, emoji: string })
...
async function saveReactionRoles(messageId, roleData) {
  try {
    await db.ref(`reactionRoles/${messageId}`).set(roleData);
    console.log(`✅ Saved reaction role for message ${messageId}`);
  } catch (error) {
...
async function getReactionRoles(messageId) {
  try {
    const snapshot = await db.ref(`reactionRoles/${messageId}`).once('value');
    return snapshot.val() || null;
  } catch (error) {
...
async function deleteReactionRole(messageId) {
  try {
    await db.ref(`reactionRoles/${messageId}`).remove();
    console.log(`✅ Deleted reaction role for message ${messageId}`);
  } catch (error) {
...
client.once('ready', async () => {
  ...
  try {
    ...
    // Load reaction roles
    const reactionRolesSnapshot = await db.ref('reactionRoles').once('value');
    const reactionRolesData = reactionRolesSnapshot.val() || {};
    
    for (const [messageId, roleData] of Object.entries(reactionRolesData)) {
      reactionRoles.set(messageId, roleData);
    }
    
    console.log(`✅ Loaded data for ${Object.keys(guildsData).length} guilds and ${Object.keys(reactionRolesData).length} reaction roles from Firebase`);
  } catch (error) {
...
client.on('ready', async () => {
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('addrole')
      .setDescription('Add a role to a specific user or all members (Requires Administrator)')
      .addStringOption(option =>
        option.setName('mode')
    ...
    new SlashCommandBuilder()
      ...
      .setDescription('Create a reaction role message (Requires Administrator)')
      .addStringOption(option =>
        option.setName('title')
      ...
      .addStringOption(option =>
        option.setName('emoji')
          .setDescription('Emoji for the reaction (use emoji or emoji ID)')
          .setRequired(true)
      )
      .addStringOption(option =>
        option.setName('color')
    ...
    new SlashCommandBuilder()
      .setName('removerole')
      .setDescription('Remove a role from a specific user or all members (Owner/Admin only)')
      .addStringOption(option =>
        option.setName('mode')
    ...
    new SlashCommandBuilder()
      ...
      .setDescription('Toggle channel restriction (commands only) for a channel (Owner/Admin only)')
      .addChannelOption(option =>
        option.setName('channel')
...
client.on('messageCreate', async message => {
  ...
  if (guildRestrictedChannels && guildRestrictedChannels.has(message.channelId)) {
    ...
    // Allow server owner and administrators to send messages
    if (message.member && (message.member.permissions.has(PermissionFlagsBits.Administrator) || message.guild.ownerId === message.author.id)) return;
    ...
    // Delete non-command messages and send warning
    try {
      await message.delete();
...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  // Get reaction role data from Firebase
  const roleData = await getReactionRoles(messageId);
  
  if (!roleData) return;

  const guild = reaction.message.guild;
  if (!guild || guild.id !== roleData.guildId) return;
  ...
  try {
    ...
    if (!role) {
      console.log(`Role ${roleData.roleId} not found, cleaning up reaction role data`);
      await deleteReactionRole(messageId);
      return;
    }

    // Enhanced emoji matching - handle both custom and unicode emojis
    ...
    console.log(`🔍 Debug - Stored emoji: "${roleData.emoji}" (length: ${roleData.emoji.length})`);
    
    if (reaction.emoji.id) {
      // Custom emoji - compare stored emoji with reaction emoji ID
      emojiMatch = roleData.emoji === reaction.emoji.id;
      console.log(`🔍 Custom emoji match: stored("${roleData.emoji}") === reaction("${reaction.emoji.id}") = ${emojiMatch}`);
    } else {
      // Unicode emoji - compare stored emoji with reaction emoji name (which is the unicode character)
      const reactionEmojiName = reaction.emoji.name; // This should be the actual unicode character like 🔥
      const reactionEmojiNative = reaction.emoji.toString(); // This should also be 🔥
      
      // For unicode emojis, reaction.emoji.name should be the actual emoji character
      emojiMatch = roleData.emoji === reactionEmojiName || roleData.emoji === reactionEmojiNative;
      
      console.log(`🔍 Unicode emoji details:`);
      console.log(`  - Stored: "${roleData.emoji}" (charCodes: ${Array.from(roleData.emoji).map(c => c.charCodeAt(0)).join(', ')})`);
      console.log(`  - Reaction name: "${reactionEmojiName}" (charCodes: ${Array.from(reactionEmojiName).map(c => c.charCodeAt(0)).join(', ')})`);
      console.log(`  - Reaction native: "${reactionEmojiNative}" (charCodes: ${Array.from(reactionEmojiNative).map(c => c.charCodeAt(0)).join(', ')})`);
      console.log(`  - Match result: ${emojiMatch}`);
    }

    if (emojiMatch && !member.roles.cache.has(roleData.roleId)) {
      await member.roles.add(role);
      console.log(`✅ Added role ${role.name} to ${user.tag} via reaction`);
    } else if (emojiMatch) {
      console.log(`ℹ️ User ${user.tag} already has role ${role.name}`);
    } else {
      console.log(`❌ Emoji mismatch - no role assigned`);
    }
  } catch (error) {
...
client.on('messageReactionRemove', async (reaction, user) => {
  ...
  try {
    ...
    console.log(`🔍 Reaction Remove Debug - Stored emoji: "${roleData.emoji}"`);
    
    if (reaction.emoji.id) {
      // Custom emoji - compare stored emoji with reaction emoji ID
      emojiMatch = roleData.emoji === reaction.emoji.id;
      console.log(`🔍 Custom emoji remove match: stored("${roleData.emoji}") === reaction("${reaction.emoji.id}") = ${emojiMatch}`);
    } else {
      // Unicode emoji - compare stored emoji with reaction emoji name (which is the unicode character)
      const reactionEmojiName = reaction.emoji.name;
      const reactionEmojiNative = reaction.emoji.toString();
      
      emojiMatch = roleData.emoji === reactionEmojiName || roleData.emoji === reactionEmojiNative;
      console.log(`🔍 Unicode emoji remove match: stored("${roleData.emoji}") === reaction("${reactionEmojiName}") OR ("${reactionEmojiNative}") = ${emojiMatch}`);
    }
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'embedcreate') {
    ...
    try {
      ...
      // Send the custom embed directly
      await interaction.reply({ embeds: [embed] });
  ...
  if (interaction.commandName === 'reactionrole') {
    ...
    if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.AddReactions)) {
      return interaction.reply({
        content: '<:no:1393890945929318542> I don\'t have permission to add reactions.',
        ephemeral: true
      });
    }
    ...
    try {
      ...
      try {
        ...
        // Parse emoji to get the correct identifier for storage
        let storedEmoji;
        if (emoji.includes(':')) {
          // Custom emoji format: <:name:id> or <a:name:id>
          const emojiMatch = emoji.match(/<a?:([^:]+):(\d+)>/);
          if (emojiMatch) {
            storedEmoji = emojiMatch[2]; // Store just the ID for custom emojis
            console.log(`📝 Storing custom emoji ID: ${storedEmoji} from ${emoji}`);
          } else {
            storedEmoji = emoji; // Fallback to original
          }
        } else {
          // Unicode emoji - store as is
          storedEmoji = emoji;
          console.log(`📝 Storing unicode emoji: ${storedEmoji}`);
        }
        ...
        reactionRoles.set(message.id, roleData);
        await saveReactionRoles(message.id, roleData);

        console.log(`✅ Created reaction role for ${role.name} with emoji ${storedEmoji} (original: ${emoji})`);
      } catch (reactionError) {
        console.error('Failed to add reaction:', reactionError);
        await interaction.followUp({
          content: '⚠️ Reaction role message created but I couldn\'t add the reaction. Please add it manually.',
          ephemeral: true
        });
      }
    } catch (error) {
      ...
      await interaction.reply({
        content: '<:no:1393890945929318542> Failed to create reaction role message. Please check your inputs and try again.',
        ephemeral: true
      });
    }
  }
      ...
      case 'restrict':
        ...
        embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Bypassed by', value: 'Server Owner and Administrators', inline: false }
          )
          .setTimestamp()
          .setFooter({
