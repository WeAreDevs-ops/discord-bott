...
// Helper function to get Hugging Face roast reply
async function getRoastReply(userMessage, username) {
    try {
        ...
        console.log('🔍 Making Hugging Face request for user:', username);
        ...
        console.log('🔍 Hugging Face Response Content-Type:', response.headers.get('content-type'));

        // Handle different error cases first
        if (!response.ok) {
            const responseText = await response.text();
            console.log('🔍 Hugging Face Error Response:', responseText);
            
            if (response.status === 401) {
                console.error('❌ Hugging Face API Error: Invalid API key');
                return "My API key is fake! Someone scammed me! 🎭";
            } else if (response.status === 429) {
                console.error('❌ Hugging Face API Error: Rate limit exceeded');
                return "I'm talking too much! Even I need to chill sometimes! 😎";
            } else if (response.status === 503) {
                console.error('❌ Hugging Face API Error: Model loading');
                return "My brain is still booting up! Give me a sec! 🤖";
            } else {
                console.error('❌ Hugging Face API Error:', response.status, responseText);
        ...
        try {
            ...
            console.log('🔍 Hugging Face Parsed Data:', JSON.stringify(data, null, 2));
        } catch (parseError) {
            console.error('❌ Failed to parse Hugging Face response as JSON:', parseError.message);
            return "My brain got scrambled! The response was gibberish! 🤯";
        }

        // Extract reply from DialoGPT response format
        let reply = data?.generated_text || data?.conversation?.generated_text || data?.[0]?.generated_text;
        ...
        console.error('❌ Hugging Face API error (catch block):', error.message);
...
client.on('messageCreate', async message => {
  ...
  if (message.channelId === '1393907304327413851') {
    ...
    try {
      ...
      // Get Hugging Face roast response
      const roastMessage = await getRoastReply(message.content, message.author.username);
