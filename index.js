...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  try {
    ...
    // Simple emoji matching - compare stored emoji with reaction emoji
    ...
    if (reaction.emoji.id) {
      ...
      // Unicode emoji - compare stored emoji with reaction emoji name
      emojiMatch = roleData.emoji === reaction.emoji.name;
    }

    console.log(`🔍 Debug - Stored: ${roleData.emoji}, Reaction: ${reaction.emoji.id || reaction.emoji.name}, Match: ${emojiMatch}`);
  ...
  if (interaction.commandName === 'reactionrole') {
    ...
    try {
      ...
      try {
        ...
        // Wait for the reaction to be added, then capture the actual emoji format
        await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second for reaction to process
        
        // Fetch the message again to get the actual reaction
        const updatedMessage = await interaction.fetchReply();
        const addedReaction = updatedMessage.reactions.cache.first();
        
        let storedEmoji;
        if (addedReaction && addedReaction.emoji.id) {
          ...
          // Fallback to original emoji if no reaction found
          storedEmoji = emoji.includes(':') ? emoji.match(/<a?:([^:]+):(\d+)>/)?.[2] || emoji : emoji;
        }
