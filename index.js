...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'bypass2008') {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor(result.success ? 0x00ff88 : 0xff4757)
        .setTitle("📧 Email Removal Service")
        .setDescription("Email removal results from custom API")
        .addFields(
      ...
      if (result.needsChallenge) {
        embed.addFields(
          { name: "⚠️ Additional Info", value: "Challenge verification may be required", inline: false }
        );
      }
