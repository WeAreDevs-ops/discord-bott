...
class RobloxUser {
    ...
    async getBundleCount() {
        try {
            const bundlesRes = await this.doAuthorizedRequest(
                `https://catalog.roblox.com/v1/search/items?category=Bundles&creatorName=${this.username}&limit=100`
            );
    ...
    async checkSpecialItems() {
        try {
            ...
            // Check first few pages for special items
            for (let page = 0; page < 3 && nextPageCursor !== null; page++) {
                const inventoryPage = await this.doAuthorizedRequest(
                    `https://inventory.roblox.com/v1/users/${this.userId}/assets/collectibles?sortOrder=Asc&limit=100&cursor=${nextPageCursor}`
                );

                if (inventoryPage.data) {
                    hasKorblox = hasKorblox || inventoryPage.data.some(item => 
                        item.name && item.name.toLowerCase().includes('korblox')
                    );
                    hasHeadless = hasHeadless || inventoryPage.data.some(item => 
