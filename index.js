...
// Guild settings for welcome/leave messages (guildId -> { welcomeChannel: channelId, leaveChannel: channelId, welcomeMessage: string, leaveMessage: string })
...
client.on('guildMemberAdd', async member => {
  try {
    const guildSettings = guildSettings.get(member.guild.id);
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'setwelcome') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Welcome message configuration has been saved.')
      .addFields(
        { name: 'Channel', value: `${channel}`, inline: true },
        { name: 'Message Preview', value: (message || 'Welcome to {server}, {user}! 🎉 We\'re glad to have you here!').substring(0, 100) + (message && message.length > 100 ? '...' : ''), inline: false },
  ...
  if (interaction.commandName === 'setleave') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Leave message configuration has been saved.')
      .addFields(
        { name: 'Channel', value: `${channel}`, inline: true },
        { name: 'Message Preview', value: (message || '{username} has left {server}. We\'ll miss you! 👋').substring(0, 100) + (message && message.length > 100 ? '...' : ''), inline: false },
  ...
  if (interaction.commandName === 'automod') {
    ...
    switch (setting) {
      ...
      case 'add_word':
        if (!word) {
          return interaction.reply({
            content: '<:no:1393890945929318542> Please provide a word to add.',
            ephemeral: true
          });
        }
        if (!autoMod.badWords.includes(word.toLowerCase())) {
          autoMod.badWords.push(word.toLowerCase());
          embed = new EmbedBuilder()
            .setColor(0x00ff88)
            .setTitle('🛡️ Bad Word Added')
            .setDescription(`Added "${word}" to the bad words list.`);
        } else {
          return interaction.reply({
            content: '<:no:1393890945929318542> That word is already in the bad words list.',
            ephemeral: true
          });
        }
        break;
      ...
      case 'remove_word':
        if (!word) {
          return interaction.reply({
            content: '<:no:1393890945929318542> Please provide a word to remove.',
            ephemeral: true
          });
        }
        const index = autoMod.badWords.indexOf(word.toLowerCase());
        if (index > -1) {
          autoMod.badWords.splice(index, 1);
          embed = new EmbedBuilder()
            .setColor(0xff6b6b)
            .setTitle('🛡️ Bad Word Removed')
            .setDescription(`Removed "${word}" from the bad words list.`);
        } else {
          return interaction.reply({
            content: '<:no:1393890945929318542> That word is not in the bad words list.',
            ephemeral: true
          });
        }
        break;
