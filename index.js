...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  try {
    ...
    // Enhanced emoji matching logic for all emoji types
    ...
    console.log(`🔍 Debug - Stored emoji: ${roleData.emoji}`);
    
    if (reaction.emoji.id) {
      // Custom emoji (including Discord custom emojis)
      ...
      const reactionEmojiFormatted = `<${reaction.emoji.animated ? 'a' : ''}:${reactionEmojiName}:${reactionEmojiId}>`;
      
      // Check multiple possible formats
      emojiMatch = roleData.emoji === reactionEmojiId || 
                   roleData.emoji === reactionEmojiString ||
                   roleData.emoji === reactionEmojiFormatted ||
                   roleData.emoji === `<:${reactionEmojiName}:${reactionEmojiId}>` ||
                   roleData.emoji === `<a:${reactionEmojiName}:${reactionEmojiId}>` ||
                   roleData.emoji.includes(reactionEmojiId) ||
                   roleData.emoji === reaction.emoji.identifier;
    } else {
      // Unicode emoji (standard keyboard emojis)
      const reactionEmojiName = reaction.emoji.name;
      emojiMatch = roleData.emoji === reactionEmojiName ||
                   roleData.emoji === reaction.emoji.toString() ||
                   roleData.emoji === reaction.emoji.identifier;
    }

    console.log(`🔍 Debug - Emoji match result: ${emojiMatch}`);
  ...
  if (interaction.commandName === 'reactionrole') {
    ...
    try {
      ...
      try {
        ...
        // Store reaction role data with comprehensive emoji format handling
        let storedEmoji = emoji;
        
        // Try to get the actual emoji object from the reaction
        const messageReactions = message.reactions.cache;
        const addedReaction = messageReactions.find(r => r.emoji.toString() === emoji || r.emoji.name === emoji || r.emoji.id === emoji);
        
        if (addedReaction) {
          if (addedReaction.emoji.id) {
            // Custom emoji - store the ID for most reliable matching
            storedEmoji = addedReaction.emoji.id;
          } else {
            // Unicode emoji - store the name/unicode
            storedEmoji = addedReaction.emoji.name;
          }
        }
