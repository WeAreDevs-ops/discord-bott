...
client.on('guildMemberRemove', async member => {
  try {
    ...
    // First send the plain text message with user mention
    const plainMessage = leaveMessage
      .replace(/{user}/g, `<@${member.id}>`)
      .replace(/{username}/g, member.user.username)
      .replace(/{server}/g, member.guild.name)
      .replace(/{membercount}/g, memberCountWithSuffix);

    await leaveChannel.send(plainMessage);

    // Then send the embed
    const embedMessage = leaveMessage
      .replace(/{user}/g, `<@${member.id}>`)
      .replace(/{username}/g, `**${member.user.username}**`)
      .replace(/{server}/g, `**${member.guild.name}**`)
      .replace(/{membercount}/g, `**${memberCountWithSuffix}**`);
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'setwelcome') {
    ...
    // Create example welcome message and embed
    const exampleMessage = (message || 'Welcome {user} to {server}! You are the {membercount} member!')
  ...
  if (interaction.commandName === 'setleave') {
    ...
    // Create example leave message and embed
    const exampleMessage = (message || '{username} has left {server}. We\'ll miss you! 👋')
      .replace(/{username}/g, `**${interaction.user.username}**`)
      .replace(/{server}/g, `**${interaction.guild.name}**`)
