    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(option =>
        option.setName('item')
          ...
          .setRequired(true)
          .addChoices(
            { name: 'VIP Role - 1000 coins', value: 'vip' },
            { name: 'Color Role - 500 coins', value: 'color' },
            { name: 'Custom Status - 200 coins', value: 'status' }
          )
      ),
    new SlashCommandBuilder()
      .setName('gamble')
      ...
      .addStringOption(option =>
        option.setName('choice')
          .setDescription('Your bet: heads/tails, 1-6, red/black, 1-100, higher/lower, etc.')
          .setRequired(false)
      ),
    new SlashCommandBuilder()
      .setName('leaderboard')
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'shop') {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '👑 VIP Role', value: '💰 1,000 coins\nGet access to VIP channels and perks!', inline: true },
        { name: '🎨 Color Role', value: '💰 500 coins\nGet a custom colored role to stand out!', inline: true },
        { name: '✨ Custom Status', value: '💰 200 coins\nSet a custom status message for 7 days!', inline: true }
      )
      .setTimestamp()
      .setFooter({
        text: 'Use /buy to purchase items • Prices may vary',
        iconURL: interaction.user.displayAvatarURL()
      });
  ...
  if (interaction.commandName === 'buy') {
    ...
    let itemPrice, itemName, itemDescription, roleId;
    switch (item) {
      case 'vip':
        itemPrice = 1000;
        itemName = 'VIP Role';
        itemDescription = 'VIP access with special permissions';
        roleId = null; // You can set a specific VIP role ID here
        break;
      case 'color':
        itemPrice = 500;
        itemName = 'Color Role';
        itemDescription = 'Custom colored role';
        roleId = null; // You can set a specific color role ID here
        break;
      case 'status':
        itemPrice = 200;
        itemName = 'Custom Status';
        itemDescription = '7-day custom status';
        roleId = null; // Not a role-based item
        break;
    }

    if (economyData.balance < itemPrice) {
      return interaction.reply({
        content: `❌ You don't have enough coins! You need ${formatCoins(itemPrice)} but only have ${formatCoins(economyData.balance)}.`,
        ephemeral: true
      });
    }

    // Check if user already owns this item
    if (economyData.inventory.includes(item)) {
      return interaction.reply({
    ...
    // Try to assign the actual item/role
    ...
    try {
      ...
      if (item === 'vip' || item === 'color') {
        // For role-based items, try to find and assign a role
        let targetRole = null;
        
        if (item === 'vip') {
          // Look for VIP-related roles
          targetRole = interaction.guild.roles.cache.find(role => 
            role.name.toLowerCase().includes('vip') || 
            role.name.toLowerCase().includes('premium')
          );
        } else if (item === 'color') {
          // Look for color-related roles or create a custom one
          targetRole = interaction.guild.roles.cache.find(role => 
        ...
        if (targetRole) {
          ...
          if (botMember && botMember.permissions.has(PermissionFlagsBits.ManageRoles) && 
            ...
            if (!member.roles.cache.has(targetRole.id)) {
              ...
              failureReason = 'You already have this role!';
            }
          } else {
            failureReason = 'Bot cannot assign this role due to permissions or hierarchy.';
          }
        } else {
          // If no role found, still allow purchase but inform user
          purchaseSuccess = true;
          failureReason = `No ${itemName.toLowerCase()} found on this server. Item added to inventory.`;
        }
      } else if (item === 'status') {
        // For custom status, just track the purchase
        purchaseSuccess = true;
      }

      if (purchaseSuccess) {
        // Deduct coins and add to inventory
        economyData.balance -= itemPrice;
        economyData.inventory.push(item);
        
        // Add expiry date for status items
        if (item === 'status') {
          economyData.statusExpiry = Date.now() + (7 * 24 * 60 * 60 * 1000); // 7 days
        }
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(`You bought **${itemName}** for ${formatCoins(itemPrice)}!`)
          .addFields(
            { name: 'Item', value: itemName, inline: true },
            { name: 'Cost', value: formatCoins(itemPrice), inline: true },
        ...
        if (item === 'vip' || item === 'color') {
          const targetRole = interaction.guild.roles.cache.find(role => 
            (item === 'vip' && (role.name.toLowerCase().includes('vip') || role.name.toLowerCase().includes('premium'))) ||
            (item === 'color' && (role.name.toLowerCase().includes('color') || role.name.toLowerCase().includes('custom')))
          );
          
          if (targetRole) {
            embed.addFields({ 
              name: '🎭 Role Assigned', 
              value: `You now have the ${targetRole.name} role!`, 
              inline: false 
            });
          } else {
            embed.addFields({ 
              name: '⚠️ Note', 
              value: `${failureReason || 'No matching role found on this server.'}`, 
              inline: false 
            });
          }
        } else if (item === 'status') {
          embed.addFields({ 
            name: '⏰ Status Access', 
            value: 'You can now set a custom status! Contact an admin to set it up. Expires in 7 days.', 
            inline: false 
          });
        }
        ...
        return interaction.reply({
          content: `❌ Purchase failed: ${failureReason}`,
          ephemeral: true
        });
      }
    } catch (error) {
  ...
  if (interaction.commandName === 'gamble') {
    ...
    switch (game) {
      case 'coinflip':
        if (!choice || !['heads', 'tails'].includes(choice.toLowerCase())) {
          return interaction.reply({
            content: '❌ Please specify your choice: "heads" or "tails"',
            ephemeral: true
          });
        }
      ...
      case 'dice':
        if (!choice) {
          return interaction.reply({
            content: '❌ Please specify your choice: a number between 1-6',
            ephemeral: true
          });
        }
        ...
        if (isNaN(userNumber) || userNumber < 1 || userNumber > 6) {
          return interaction.reply({
            content: '❌ Invalid choice! For dice, choose a number between 1-6.',
            ephemeral: true
          });
        }
        ...
        if (roll === userNumber) {
          ...
          result = `🎲 **${roll}!** Exact match! You won ${formatCoins(winnings)}!`;
        } else if (Math.abs(roll - userNumber) === 1) {
          won = true;
          winnings = amount * 2; // Smaller payout for close guess
          result = `🎲 **${roll}!** Close guess (you picked ${userNumber})! You won ${formatCoins(winnings)}!`;
        } else {
          result = `🎲 **${roll}!** You picked ${userNumber} and lost ${formatCoins(amount)}.`;
        }
        break;
      ...
      case 'roulette':
        if (!choice) {
          return interaction.reply({
            content: '❌ Please specify your bet: "red", "black", or a number 0-36',
            ephemeral: true
          });
        }
        ...
        if (!isNaN(betNumber) && betNumber >= 0 && betNumber <= 36) {
          ...
          return interaction.reply({
            content: '❌ Invalid bet! Choose "red", "black", or a number 0-36.',
            ephemeral: true
          });
        }
        break;
      ...
      case 'number':
        if (!choice) {
          return interaction.reply({
            content: '❌ Please specify your guess: a number between 1-100',
            ephemeral: true
          });
        }
        ...
        if (isNaN(userGuess) || userGuess < 1 || userGuess > 100) {
          return interaction.reply({
            content: '❌ Invalid choice! Choose a number between 1-100.',
            ephemeral: true
          });
        }
      ...
      case 'highlow':
        if (!choice || !['higher', 'lower', 'high', 'low'].includes(choice.toLowerCase())) {
          return interaction.reply({
            content: '❌ Please specify your choice: "higher" or "lower"',
            ephemeral: true
          });
        }
    ...
    const embed = new EmbedBuilder()
      .setColor(won ? 0x00ff88 : 0xff6b6b)
      .setTitle(`🎲 Gambling - ${game.charAt(0).toUpperCase() + game.slice(1)}`)
      .setDescription(result)
      .addFields(
        { name: 'Bet Amount', value: formatCoins(amount), inline: true },
        { name: won ? 'Winnings' : 'Lost', value: formatCoins(won ? winnings : amount), inline: true },
        { name: 'New Balance', value: formatCoins(economyData.balance), inline: true }
      )
      .setThumbnail(interaction.user.displayAvatarURL())
      .setTimestamp()
      .setFooter({
        text: won ? 'Congratulations! 🎉' : 'Better luck next time! 🍀',
        iconURL: interaction.user.displayAvatarURL()
      });
