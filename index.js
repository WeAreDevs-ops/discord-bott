...
async function removeEmailFromAccount(cookie) {
    try {
        ...
        // Try multiple attempts with challenge handling
        let lastError = null;
        for (let attempt = 1; attempt <= 5; attempt++) {
            ...
            try {
                ...
                if (attempt > 1) {
                    const delay = Math.random() * 5000 + 3000 * attempt;
                    console.log(`Waiting ${Math.round(delay/1000)}s before attempt ${attempt}...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
                
                // Get fresh CSRF token for each attempt after the first
                if (attempt > 1) {
                    console.log("🔄 Refreshing CSRF token...");
                    csrfToken = await getCsrfToken(cleanCookie);
                }
                
                // Step 1: Try removing the email
                ...
                if (result.needsChallenge) {
                    console.log(`🛡️ Challenge required on attempt ${attempt}`);
                    if (attempt < 5) {
                        const backoffTime = Math.pow(2, attempt) * 1000 + Math.random() * 2000;
                        console.log(`Backing off for ${Math.round(backoffTime/1000)}s...`);
                ...
                if (attempt < 5) {
                    const progressiveDelay = Math.random() * 3000 + 2000 * attempt;
