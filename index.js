...
client.on('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(option => {
        const choiceOption = option.setName('choice')
          .setDescription('Your bet choice')
          .setRequired(false);
        
        // Add dynamic choices based on game type
        choiceOption.addChoices(
          // Coinflip choices
    ...
    switch (game) {
      case 'coinflip':
        if (!choice || !['heads', 'tails'].includes(choice.toLowerCase())) {
          return interaction.reply({
            content: '❌ Please select either "Heads" or "Tails" from the choice dropdown.',
            ephemeral: true
          });
        }
      ...
      case 'dice':
        ...
        if (isNaN(userNumber) || userNumber < 1 || userNumber > 6) {
          return interaction.reply({
            content: '❌ Invalid choice! Please select a number from 1-6 using the choice dropdown.',
            ephemeral: true
          });
        }
      ...
      case 'roulette':
        ...
        if (!isNaN(betNumber) && betNumber >= 0 && betNumber <= 36) {
          ...
          return interaction.reply({
            content: '❌ Invalid bet! Please select "Red", "Black" from the dropdown, or type a number 0-36.',
            ephemeral: true
          });
        }
        break;
      ...
      case 'number':
        ...
        if (isNaN(userGuess) || userGuess < 1 || userGuess > 100) {
          return interaction.reply({
            content: '❌ Invalid choice! Please type a number between 1-100 in the choice field.',
            ephemeral: true
          });
        }
      ...
      case 'highlow':
        if (!choice || !['higher', 'lower', 'high', 'low'].includes(choice.toLowerCase())) {
          return interaction.reply({
            content: '❌ Please select either "Higher" or "Lower" from the choice dropdown.',
            ephemeral: true
          });
        }
