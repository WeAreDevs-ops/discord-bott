      ...
      case 'unlock':
        ...
        try {
          ...
          for (const [roleId, role] of roles) {
            ...
            try {
              // Remove the explicit SEND_MESSAGES deny permission (reset to inherit)
              await channel.permissionOverwrites.edit(roleId, {
                SendMessages: null
              });
              successCount++;
          ...
          embed = new EmbedBuilder()
            ...
            .setDescription(`${channel} has been unlocked by restoring send message permissions.`)
            .addFields(
              { name: 'Channel', value: `${channel}`, inline: true },
              { name: 'Status', value: '🔓 Unlocked', inline: true },
              { name: 'Reason', value: reason, inline: false },
              { name: 'Permissions Restored', value: `${successCount} roles affected`, inline: true },
              { name: 'Messages', value: 'All message types are now allowed', inline: false }
            )
            .setTimestamp()
            .setFooter({
