...
app.get('/auth/discord/callback', async (req, res) => {
    ...
    try {
        ...
        console.log('Token exchange payload (without secret):', { ...tokenPayload, client_secret: '[HIDDEN]' });
        ...
        if (!tokenResponse.ok || tokenData.error) {
            ...
            if (tokenData.error === 'invalid_grant') {
                ...
                return res.redirect('/?error=code_expired&message=Authorization code expired or already used');
            }
            
            return res.redirect(`/?error=token_exchange_failed&reason=${tokenData.error}&description=${encodeURIComponent(tokenData.error_description || 'Unknown error')}`);
        }
