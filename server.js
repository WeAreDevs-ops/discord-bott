...
app.post('/api/admin/remove-server', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    let guild = botClient.guilds.cache.get(serverId);
    
    // If not in cache, try to fetch it (this will work even if you're not a member)
    if (!guild) {
      console.log(`Guild ${serverId} not in cache, attempting to fetch...`);
      try {
        guild = await botClient.guilds.fetch(serverId);
        console.log(`Successfully fetched guild: ${guild.name}`);
      } catch (fetchError) {
        console.error(`Failed to fetch guild ${serverId}:`, fetchError);
        
        // If fetch fails, try using Discord REST API directly with bot token
        try {
          console.log('Attempting direct API call to leave guild...');
          ...
          if (response.ok) {
            ...
            try {
              const activity = {
                ...
                description: `Bot was removed from server (ID: ${serverId}) by admin via REST API. Reason: ${reason || 'No reason provided'}`,
                timestamp: Date.now(),
                serverId: serverId,
                serverName: 'Unknown (not accessible)',
                memberCount: 'Unknown',
            ...
            return res.json({
              success: true,
              message: `Successfully removed bot from server (ID: ${serverId}) via REST API`,
              details: {
                serverId: serverId,
                method: 'REST_API',
                reason: reason || 'No reason provided'
            ...
            console.error(`REST API leave failed: ${response.status} - ${errorText}`);
            ...
            if (response.status === 404) {
              ...
              return res.json({
                success: false,
                error: `Failed to remove bot via REST API: ${response.status}`,
                details: errorText
              });
            }
          }
        } catch (apiError) {
          console.error('Error with REST API call:', apiError);
          return res.status(500).json({
            error: 'Bot is not in this server or server is not accessible',
            details: 'Could not access server via client or REST API'
          });
        }
      }
    }

    // If we successfully got the guild object, use the standard leave method
    ...
    // Leave the guild - this works regardless of your personal permissions
    try {
      await guild.leave();
      ...
      // If leaving fails, try REST API as fallback
      try {
        console.log('Guild.leave() failed, trying REST API fallback...');
        const response = await fetch(`https://discord.com/api/v10/users/@me/guilds/${serverId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bot ${process.env.BOT_TOKEN}`,
        ...
        if (response.ok) {
          console.log(`Successfully left guild ${serverId} via REST API fallback`);
          return res.json({
            success: true,
            message: `Successfully removed bot from ${guildName} (used REST API fallback)`,
            details: {
              serverName: guildName,
              serverId: serverId,
              memberCount: memberCount,
          ...
          const errorText = await response.text();
          console.error(`REST API fallback failed: ${response.status} - ${errorText}`);
        }
      } catch (apiError) {
        console.error('REST API fallback error:', apiError);
      }
