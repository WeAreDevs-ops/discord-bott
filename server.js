...
app.get('/auth/discord/callback', async (req, res) => {
    ...
    try {
        ...
        // Store user data in Firebase for persistent login
        try {
            ...
            await db.ref(`user_sessions/${userData.id}`).set(userSessionData);
            console.log(`Stored persistent session for user ${userData.username}`);
        } catch (error) {
            console.error('Error storing user session in Firebase:', error);
        }
...
app.get('/auth/logout', async (req, res) => {
    ...
    if (userId) {
        ...
        try {
            await db.ref(`user_sessions/${userId}`).remove();
            console.log(`User ${userId} logged out and Firebase session cleared`);
        } catch (error) {
...
async function requireAuth(req, res, next) {
    ...
    // If no session, check Firebase for persistent login
    ...
    if (authHeader || userIdFromCookie) {
        try {
            ...
            if (authHeader && authHeader.startsWith('Bearer ')) {
                // Extract user ID from token (you might want to decode a JWT here)
            ...
            if (userId) {
                // Check Firebase for user session
                const userSessionSnapshot = await db.ref(`user_sessions/${userId}`).once('value');
                const userSessionData = userSessionSnapshot.val();

                if (userSessionData && userSessionData.token_expires_at > Date.now()) {
                    // Restore session from Firebase
                    req.session.user = {
                        id: userSessionData.id,
                        username: userSessionData.username,
                        discriminator: userSessionData.discriminator,
                        avatar: userSessionData.avatar,
                        email: userSessionData.email,
                        verified: userSessionData.verified,
                        access_token: userSessionData.access_token,
                        token_expires_at: userSessionData.token_expires_at,
                        granted_scopes: userSessionData.granted_scopes
                    };
                    req.session.guilds = userSessionData.guilds;

                    // Update userTokens map
                    userTokens.set(userId, {
                        access_token: userSessionData.access_token,
                        refresh_token: userSessionData.refresh_token,
                        token_type: 'Bearer',
                        scope: userSessionData.granted_scopes.join(' '),
                        expires_at: userSessionData.token_expires_at,
                        granted_scopes: userSessionData.granted_scopes
                    });

                    console.log(`Restored session from Firebase for user ${userSessionData.username}`);
                    return next();
                }
            }
        } catch (error) {
...
app.get('/api/servers', requireAuth, async (req, res) => {
    try {
        ...
        // Check bot presence from Firebase database
        const guildsWithBotInfo = await Promise.all(adminGuilds.map(async (guild) => {
            try {
                // Check if guild exists in Firebase and if bot is present
                const guildSnapshot = await db.ref(`guilds/${guild.id}`).once('value');
                const guildData = guildSnapshot.val();

                // Get bot presence from database, fallback to Discord API check
                const botPresent = guildData?.botPresent || false;

                // Get member count from Discord API if bot is present, otherwise from database or OAuth data
                let memberCount = guild.approximate_member_count || null;
                if (botPresent && client) {
                    const botGuild = client.guilds.cache.get(guild.id);
                    if (botGuild) {
                        memberCount = botGuild.memberCount;
                    }
                } else if (guildData?.memberCount) {
                ...
                return {
                    ...
                    displayMemberCount: memberCount ? memberCount.toLocaleString() : 'Unknown',
                    // Add additional data from Firebase if available
                    botJoinedAt: guildData?.joinedAt || null,
                    lastUpdated: guildData?.lastUpdated || null
                };
            } catch (error) {
                console.error(`Error checking bot presence for guild ${guild.id}:`, error);
                // Fallback to Discord API check
...
app.get('/api/dashboard/:guildId/roles', requireAuth, requireGuildAdmin, async (req, res) => {
    ...
    try {
        ...
        const roles = rolesCollection
            .filter(role => role && role.id !== guild.id) // Exclude @everyone
            .map(role => ({
                id: role.id,
                name: role.name,
                ...
                managed: role.members ? role.members.size : 0,
...
app.get('/auth/discord/bot-invite', (req, res) => {
    ...
    let authUrl = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&response_type=${responseType}&redirect_uri=${redirectUri}&scope=${scope}&permissions=${permissions}&state=${encodeURIComponent(state)}`;
...
app.get('/api/session/check', async (req, res) => {
    try {
        const userId = req.session.user?.id || req.cookies?.user_id;
        ...
        if (userSessionData && userSessionData.token_expires_at > Date.now()) {
            return res.json({ 
                ...
                expiresAt: userSessionData.token_expires_at
            });
        } else {
            return res.json({ authenticated: false });
        }
...
app.post('/api/session/extend', requireAuth, async (req, res) => {
    try {
        ...
        await db.ref(`user_sessions/${userId}/token_expires_at`).set(newExpiry);
        await db.ref(`user_sessions/${userId}/lastActivity`).set(Date.now());
