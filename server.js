...
app.get('/api/servers', requireAuth, async (req, res) => {
    try {
        ...
        console.log(`Bot client status: ready=${client?.isReady()}, guilds=${client?.guilds?.cache?.size || 0}`);

        // Check bot presence directly from Discord client with proper detection
        const enhancedGuildsInfo = await Promise.all(adminGuilds.map(async (guild) => {
            let botGuild = null;
            let botPresent = false;
            let memberCount = guild.approximate_member_count || 0;
            let displayMemberCount = guild.approximate_member_count ? guild.approximate_member_count.toLocaleString() : 'Unknown';
            ...
            if (client && client.isReady()) {
                try {
                    ...
                    if (!botGuild) {
                        console.log(`Guild ${guild.id} not in cache, attempting to fetch...`);
                        try {
                            botGuild = await client.guilds.fetch(guild.id);
                            console.log(`Successfully fetched guild ${guild.id} from API`);
                        } catch (fetchError) {
                            console.log(`Failed to fetch guild ${guild.id}:`, fetchError.message);
                            botGuild = null;
                        }
                    }

                    // If we have the guild, bot is present
                    if (botGuild) {
                        botPresent = true;
                        memberCount = botGuild.memberCount;
                        displayMemberCount = botGuild.memberCount.toLocaleString();
                        botJoinedAt = botGuild.joinedTimestamp;
                        console.log(`Bot confirmed present in guild ${guild.name} (${guild.id})`);
                    } else {
                        console.log(`Bot not present in guild ${guild.name} (${guild.id})`);
                    }
                } catch (error) {
                    console.error(`Error checking bot presence for guild ${guild.id}:`, error.message);
                    botPresent = false;
                }
            } else {
                console.log('Discord client not ready, assuming bot not present');
                botPresent = false;
            }
