...
// Firebase setup - use the existing firebase.js module
...
class FirebaseSessionStore extends EventEmitter {
    ...
    async get(sessionId, callback) {
        try {
            const snapshot = await db.ref(`${this.prefix}${sessionId}`).once('value');
    ...
    async set(sessionId, session, callback) {
        try {
            await db.ref(`${this.prefix}${sessionId}`).set(session);
            callback(null);
        } catch (error) {
            callback(error);
        }
    ...
    async destroy(sessionId, callback) {
        try {
            await db.ref(`${this.prefix}${sessionId}`).remove();
            callback(null);
        } catch (error) {
            callback(error);
        }
    ...
    async touch(sessionId, session, callback) {
        ...
        try {
            await db.ref(`${this.prefix}${sessionId}`).update({
                ...session,
                lastAccess: Date.now()
            });
            callback(null);
    ...
    async length(callback) {
        try {
            const snapshot = await db.ref(this.prefix.slice(0, -1)).once('value');
    ...
    async clear(callback) {
        try {
            await db.ref(this.prefix.slice(0, -1)).remove();
            callback(null);
        } catch (error) {
            callback(error);
        }
...
// Configure session middleware with Firebase store
app.use(session({
