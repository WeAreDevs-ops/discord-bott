...
function generateBotInviteLink(guildId) {
  ...
  return `https://discord.com/api/oauth2/authorize?client_id=${clientId}&permissions=${permissions}&guild_id=${guildId}&scope=${scope}&response_type=code&redirect_uri=${encodeURIComponent(DISCORD_REDIRECT_URI)}&disable_guild_select=true`;
}
...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try {
    ...
    if (guild_id) {
      // Wait a moment for Discord to process the bot addition
      setTimeout(() => {
...
app.get('/dashboard', requireAuth, async (req, res) => {
  try {
    ...
    // Get bot guild IDs from the bot client
    let botGuildIds = [];
    try {
      // Import the bot client from index.js
      ...
      console.log('Bot client not available, falling back to API check');
      // Fallback to API method if bot client is not accessible
    }
    ...
    const guildsWithBotStatus = await Promise.all(
      managedGuilds.map(async (guild) => {
        ...
        // First try using bot client cache
        ...
        return {
          ...guild,
          botPresent,
          inviteLink: !botPresent ? generateBotInviteLink(guild.id) : null
        };
      })
    );
...
app.post('/api/guild/:id/embed/create', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Failed to send embed to channel. Please check the channel ID and bot permissions.' });
    }
