...
app.get('/guild/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Fetch guild settings from Firebase
    const guildSettings = await getGuildSettings(guildId);
    const autoModSettings = await getAutoModSettings(guildId);
    const autoRoleSettings = await getAutoRoleSettings(guildId);
    const commandAssignments = await getCommandAssignments(guildId);
    const restrictedChannels = await getRestrictedChannels(guildId);
