...
app.post('/api/guild/:id/embed/create', requireAuth, async (req, res) => {
  const guildId = req.params.id;
  const { title, description, color, thumbnail, image, footer, timestamp, buttons, webhookUrl } = req.body;
  ...
  try {
    ...
    try {
      const webhookPayload = { 
        ...
        avatar_url: 'https://cdn.discordapp.com/attachments/123456789/123456789/bot-avatar.png'
      };
      
      if (components.length > 0) {
        webhookPayload.components = components;
      }
      ...
      console.log(`Embed sent via webhook, message ID: ${sentMessage.id}`);
    } catch (error) {
      console.error('Error sending embed via webhook:', error);
      return res.status(500).json({ error: 'Failed to send embed via webhook. Please check the webhook URL and try again.' });
    }
    ...
    res.json({ 
      ...
      embedId: embedId,
      messageId: sentMessage.id,
      message: 'Embed created and sent successfully via webhook'
    });
  } catch (error) {
